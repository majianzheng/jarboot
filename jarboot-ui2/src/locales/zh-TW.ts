...
export default {
  ...
  ENV_TIP: '運行時支持自動加載“工作目錄”下的.env檔案，與當前配寘項合併',
  ENVIRONMENT_VARIABLES: '環境變數',
  ENV_KEY: '變數',
  ENV_VALUE: '值',
  NEW_ENV_VARIABLE: '新增环境變數',
  EDIT_ENV_VARIABLE: '编辑环境變數',
  CONFIRM: '確認',
  PRIORITY_TIP: '多用於服務間依賴，啟動時會按照優先順序從大往小的順序啟動。 停止時順序是相反的，優先順序相同的一組會並行啟動',
  ONCE_TIP: '程式啟動後完成任務會很快自己退出的，不是長期運行的，比如一些小腳本。',
  LONG_TIME_TIP: '程式需要一直運行，比如web服務。',
  CRON_TIP: '定期執行的小程式、腳本，不是長期運行的。',
  RESTART_CRON_TIP: '長期運行的任務（比如web服務）但需要隔段時間重啓一次，重啓時會先停止服務，再啟動服務。',
  WORK_DIR_TIP: '為空時默認為服務目錄，注意這裡若配寘其它目錄而不是服務目錄時，.env檔案是加載的工作目錄中的檔案，而不是服務目錄',
  SERVICE_DIR: '服務目錄',
  VM_OPT_TIP: 'VM參數檔案，默認為服務目錄下的“boot.vmoptions”檔案，可以改成其它其它檔案',
