...
public class CommandDispatch {
    ...
    public CommandDispatch(String url, WebSocketClient client) {
        this.url = url;
        this.client = client;
    }
    public void execute(String command) {
        logger.debug("收到消息：{}", command);
        if (command.isEmpty()) {
            logger.warn("收到的消息格式错误");
            CommandResponse resp = new ResponseBuilder().setType(CommandConst.ACK_TYPE
            ).setResultMsg("协议格式错误").setResultCode(ResultCodeConst.INVALID_PARAM).getResponse();
            sendTo(resp);
            return;
        }
        ...
        if (-1 == pos) {
            ...
            cmd = command.substring(0, pos).toLowerCase();
            param = pos > command.length() - 2 ? "" : command.substring(pos + 1);
        }


        logger.debug("开始执行命令：{}, param", cmd, param);
        String body = "";
        switch (cmd) {
